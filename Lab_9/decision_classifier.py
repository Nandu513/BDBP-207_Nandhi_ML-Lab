...
from sklearn.model_selection import train_test_split, KFold  # Import functions to split data and perform cross-validation
from sklearn.preprocessing import StandardScaler  # For scaling the features
from sklearn.tree import DecisionTreeClassifier  # For building the decision tree classifier
from sklearn.metrics import accuracy_score  # To evaluate model performance using accuracy score
...
def load_data(data):
    ...
    # Map 'diagnosis' column values: 'B' to 0 and 'M' to 1 for binary classification
...
def hyper_tune(x_train, x_val, y_train, y_val):
    ...
    for depth in max_depths:
        clf = DecisionTreeClassifier(criterion="entropy", max_depth=depth)  # Create a decision tree with the specified depth
        ...
        fold_score = accuracy_score(y_val, y_pred)  # Calculate accuracy score
...
def decision_tree(data):
    ...
    x = x_train.drop(columns=['diagnosis'], axis=1)  # Features (drop the target 'diagnosis')
    y = x_train['diagnosis']  # Target variable (diagnosis)
    ...
    for i, (train_index, val_index) in enumerate(kf.split(x_tr, y)):
        ...
        # Calculate accuracy score for the fold and append it to the list of scores
    ...
    x_test_scaled = sc.transform(x_test.drop(columns=['diagnosis'], axis=1))  # Scale the test data
    y_test = x_test['diagnosis']  # Extract the target values from the test data
